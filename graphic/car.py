...
import pygame
from graphic.maps import MAP_NAVS, TRAFFIC_LAMP_POS, TRAFFIC_LAMP_COORDINATES
...
class Car(pygame.sprite.Sprite):
    ...
    def __init__(self, init_x, init_y, init_dir,init_map_num):
        pygame.sprite.Sprite.__init__(self)
        self.image = load_image("car_32_24.png")
    ...
    def calculate_distance_impediment(self, stone_status):
        if stone_status[0] == 0:
            return 2000
        pos = stone_status[1]
        distance_to_impediment = math.hypot(self.MAP_NAVS[pos][0] - self.x, self.MAP_NAVS[pos][1] - self.y)
    ...
    def update_map_nav_index(self):
        ...
        # if math.hypot(self.x - next_nav_x, self.y - next_nav_y) < 10:
            self.current_nav_index = next_nav_index


    def change_dir(self, target_dir):
        viet = 2
        bestmonster = math.fabs(target_dir - self.dir)
        def BestMonster(a):
        ...
        if target_dir in [0, 90, 180, 270] or bestmonster > 90 or not check(target_dir, self.dir):
            self.dir = target_dir
        else:
    ...
    def update(self, last_x, last_y, traffic_lamp_status, stone_status, flag,pause =False):
        if pause :
            self.set_speed(0)
            return
